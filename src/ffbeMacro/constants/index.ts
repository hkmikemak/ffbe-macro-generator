...
const calculatePosition = (value: number, baseValue: number, newBaseValue: number) => Math.floor((newBaseValue / baseValue) * value);
export const calculateWidth = (value: number, config: MacroConfig) => calculatePosition(value, 720, config.screenWidth);
export const calculateHeight = (value: number, config: MacroConfig) => calculatePosition(value, 1280, config.screenHeight);

export const getSlotPosition = (slot: number, config: MacroConfig): IPosition => {
...
export const getCommonButtonPosition = (config: MacroConfig) => {
  ...
  Object.keys(COMMON_BUTTON_POSITION).forEach((key) => {
    ...
    const newPosition = {x: calculateWidth(position.x, config), y: calculateHeight(position.y, config)};
